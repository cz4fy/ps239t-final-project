...
setwd("/Users/chelseazhou/Desktop/PS239T/12_GeoSpatial")          # set your own working directory
...
#group by variable group, telling R to group points that belong to a single group
...
length(which(world$region=="Canada"))#contains many more islands
...
#can only fill color when using geom_polygon function
# explain
...
y <- geocode("Lake Merritt, CA", source="google")
class(y)
# Point Maps 
...
head(cafe)
str(cafe)
#get address of the cafes and use geocode function to get longitude and latitude
#openstreet map api 
#can only geocode 2,500 addresses per day
map <- get_map("Berkeley", zoom=14)         #more zoomed in        # retrieve a map from Gmap API
ggmap(map)
...
getClass("Spatial")
#can combine spatial data with attribute data in Data Frame part of Class
...
#original data only has attribute data not spatial data
head(dat)
class(dat)

# extract coordinates (long, lat)
dat_coords <- cbind(dat$long, dat$lat) 
row.names(dat_coords) <- dat$name #assign cafe names to the rows of dat.coords objects
str(dat_coords)
...
str(dat_sp) 
summary(dat_sp)
#Is projects: FALSE becuase we didn't project it!
...
#coordinates extracts a row that satifies row.names = "strada"
...
library(rgdal)
# reads in GIS shapefiles into R!!!!

# crs and projection
# ### codes for plots included in the slides
setwd("/Users/chelseazhou/Desktop/PS239T/12_GeoSpatial/PS239T")
ogrInfo(".", "uc_bldgs")
...
#line up shapefiles by transforming into the same GRS 
...
data <- SpatialPointsDataFrame(dat_sp[match(row.names(dat), row.names(dat_sp)),],
                               ...
                               match.ID =TRUE) 
class(data)
#match id option set to TRUE to match attribute data to spatial data

# combine a new variable

var  <- read.csv("var.csv", stringsAsFactors=F)
#cbind desn't work in spatial dataframe
data <- cbind(data, var)
#cbind data with regular data frame
class(data)
#this is the right way to do it!!
# spCbind is the way to go!
...
#google R brewer color pallette
#swap light and dark by wrapping col=rev(colpal)
...
#check out blocks on the boundary of berkeley
dim(census)
#last slot of block data is usually set to the boundary
x <- slot(slot(slot(census, "polygons")[[1196]], "Polygons")[[1]], "coords")
x
plot(census, border="grey")
lines(x, col="red", cex=1.5, pch=16)
#reaffirms that 1196 is the last block of berkeley census data
...
#select blocks from census data, bring block out and make each block into independent spatial polygon
...
census$census.id <- NA
plot(census, border="grey")
#for loop assigns id to different blocks
...
#gDistance function two inputs, distance from barrows hall to all cafes
plot(census, border="grey")
...
#list of 0 and 1 of whether the cafes are within the buffer zone
over(buff, dataT, returnList = TRUE)
#flips the order so only returns cafes that lie within the buffer

# overlaying with oakland data
setwd("/Users/chelseazhou/Desktop/PS239T/12_GeoSpatial/PS239T/oakland_blkgrp")
